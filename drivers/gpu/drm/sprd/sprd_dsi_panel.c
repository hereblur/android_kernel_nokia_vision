...
// Modify by qiu_hai@hoperun.com for drm notifier begin
static int sprd_panel_unprepare(struct drm_panel *p)
{
	...
	evdata.data = &blank;
	drm_panel_notifier_call_chain(p, DRM_PANEL_EVENT_BLANK, &evdata);
...
static int sprd_panel_prepare(struct drm_panel *p)
{
	...
	evdata.data = &blank;
	drm_panel_notifier_call_chain(p, DRM_PANEL_EARLY_EVENT_BLANK, &evdata);
...
// Modify by qiu_hai@hoperun.com for drm notifier end
