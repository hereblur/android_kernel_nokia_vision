...
/* Modify by ye_qin for hardware info on 20220518 begin */
#include <linux/hardware_info.h>
/* Modify by ye_qin for hardware info on 20220518 end */
...
#define BQ2560X_REG_BATFET_RST_EN_SHIFT         2
#define BQ2560X_REG_BATFET_RST_DISABLE          0
...
static int
bq2560x_charger_disable_batfet_rst(struct bq2560x_charger_info *info)
{
	u8 val = BQ2560X_REG_BATFET_RST_DISABLE << BQ2560X_REG_BATFET_RST_EN_SHIFT;

	return bq2560x_update_bits(info, BQ2560X_REG_7,
				   BQ2560X_REG_BATFET_RST_EN_MASK,
				   val);
}
...
static int
bq2560x_charger_enter_hiz_mode(struct bq2560x_charger_info *info)
{
	u8 val = 1 << BQ2560X_REG_EN_HIZ_SHIFT;

	return bq2560x_update_bits(info, BQ2560X_REG_0, BQ2560X_REG_EN_HIZ_MASK, val);
}

static int
bq2560x_charger_exit_hiz_mode(struct bq2560x_charger_info *info)
{
	u8 val = 0 << BQ2560X_REG_EN_HIZ_SHIFT;

	return bq2560x_update_bits(info, BQ2560X_REG_0, BQ2560X_REG_EN_HIZ_MASK, val);
}

static int
bq2560x_charger_vbus_enable(struct bq2560x_charger_info *info, u32 enable)
{
	int ret = 0;
	u8 val;

	if (enable) {
		ret = bq2560x_charger_enter_hiz_mode(info);
	} else {
		ret = bq2560x_charger_exit_hiz_mode(info);
	}

	ret = bq2560x_read(info, BQ2560X_REG_0, &val);
	if (ret == 0) {
		dev_err(info->dev, "the BQ2560X_REG_0 is 0x%.2x\n", val);
	}

	dev_err(info->dev, "%s hiz_mode %s\n", enable ? "enable" : "disable",
...
static int bq2560x_charger_hw_init(struct bq2560x_charger_info *info)
{
	...
	ret = bq2560x_charger_disable_batfet_rst(info);
	if (ret) {
		dev_err(info->dev, "disable batfet_rst failed\n");
		return ret;
	}
				...
				const union power_supply_propval *val)
{
	...
	#ifdef WT_COMPILE_FACTORY_VERSION
	//add by jack,20220628,add ato electric quantity control,start
	case POWER_SUPPLY_PROP_VBUS_ENABLE:
		ret  = bq2560x_charger_vbus_enable(info, val->intval);
		if (ret < 0) {
			dev_err(info->dev, "failed to set vbus enable\n");
		}
		break;
	//add by jack,20220628,add ato electric quantity control,end
	#endif
