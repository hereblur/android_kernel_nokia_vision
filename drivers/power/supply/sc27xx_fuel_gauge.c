...
static bool is_calibration_mode;
					    ...
					    int pocv_cap, int pocv_uv, int *cap)
{
	...
	int ret;
/*	//前移到sc27xx_fgu_get_boot_capacity()中实现
	if (is_calibration_mode){
		*cap = pocv_cap;
		dev_err(data->dev, "Cali mode set pocv_cap to UI!\n");
		return;
	}
*/
...
static int sc27xx_fgu_get_boot_capacity(struct sc27xx_fgu_data *data, int *cap)
{
	 ...
	 */
	dev_err(data->dev, "is_first_poweron = %d, is_calibration_mode = %d\n", is_first_poweron, is_calibration_mode);
//	if (!is_first_poweron) {
	if( !((is_first_poweron)||(is_calibration_mode)) ){
	...
	*cap = pocv_cap;	//首次通电或者进校准模式，就以POCV对应的电池曲线电量为准，将pocv_cap设置为当前显示电量
	data->boot_cap = *cap;
	...
	if (is_calibration_mode) {	//校准模式设置下次开机为首次通电状态
		dev_err(data->dev, "Cali mode set next poweron to is_first_poweron!\n");
		return sc27xx_fgu_save_boot_mode(data, 0xF);//set next poweron to is_first_poweron
	}else{
	return sc27xx_fgu_save_boot_mode(data, SC27XX_FGU_NORMAIL_POWERTON);
	}
}
...
#define BMS_GAUGE_INFO		"SC2721G"	//add by wt.liuzhiqing for SCT-702, hardware info: fuel-gauge info
