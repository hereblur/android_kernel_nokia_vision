...
//add by jack for real_type on 2022.05.12 begin
static ssize_t show_real_type(struct device *dev,struct device_attribute *attr, char *buf)
{
	...
	batt_psy = power_supply_get_by_name("bq2560x_charger");
	if(batt_psy) {
...
//add by jack for real_type on 2022.05.12 end
				   ...
				   bool ws)
{
	...
	struct power_supply *psy;
	int i, rc;
	int ret_device_file = 0;	//add by jack for real_type, 2022.05.12
