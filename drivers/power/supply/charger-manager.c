...
//add by jack,20220628,add ato electric quantity control,start
#ifdef WT_COMPILE_FACTORY_VERSION
static bool ato_capacity_limited = false;
#endif
//add by jack,20220628,add ato electric quantity control,end
...
static bool is_charging(struct charger_manager *cm)
{
	...
	for (i = 0; cm->desc->psy_charger_stat[i]; i++) {
		...
		//Add by xu_shengjie@hoperun.com for VSI-676 setting maximum charge percentage func on 2022-05-31 begin
		#ifdef TARGET_PRODUCT_VSI
		if(cm->hmd_max_chg_perc_status)
			continue;
		#endif
		//Add by xu_shengjie@hoperun.com for VSI-676 setting maximum charge percentage func on 2022-05-31 end
...
//add by jack,20220628,add ato electric quantity control,start
#ifdef WT_COMPILE_FACTORY_VERSION
static bool cm_vbus_enable(struct charger_manager *cm, bool enable)
{
	union power_supply_propval val;
	struct power_supply *psy;
	int ret;
	...
	ret = power_supply_set_property(psy, POWER_SUPPLY_PROP_VBUS_ENABLE, &val);
	power_supply_put(psy);
	if (ret) {
		dev_err(cm->dev, "failed to %s primary charger vbus hiz mode, ret = %d\n",
				enable ? "enable" : "disable", ret);
		return false;
	}
...
static int cm_fixed_fchg_enable(struct charger_manager *cm)
{
	...
	#ifdef TARGET_PRODUCT_VSI
	if (cm->hmd_max_chg_perc_status) {
		dev_info(cm->dev, "%s: hmd max chg perc had been set!\n", __func__);
		return -EAGAIN;
	}
	#endif
	//Add by xu_shengjie@hoperun.com for VSI-676 setting maximum charge percentage func on 2022-05-31 end
...
static void cm_cp_state_exit(struct charger_manager *cm)
{
	...
	//Add by xu_shengjie@hoperun.com for VSI-676 setting maximum charge percentage func on 2022-05-31 begin
	#ifdef TARGET_PRODUCT_VSI
	if (!cm->charging_status && !cm->emergency_stop && !cm->force_stop_charge && !cm->hmd_max_chg_perc_status) {
	#else
...
//modify by jack, jeita policy update(20220524) begin
#if 1
static bool cm_manager_adjust_current(struct charger_manager *cm,
		int jeita_status)
{
	int term_volt, target_cur=0, batt_uV;
	...
	if (bat_temp<0 || bat_temp>=550) {
		target_cur = 0;
	} else if (bat_temp>=0 && bat_temp<180) {
	...
	} else if (bat_temp>=180 && bat_temp<420) {
		if (bat_temp<=210 && last_bat_temp<180) {
			skip_temp_record = true;
			target_cur = 900000;
		} else if (last_bat_temp>=420 && bat_temp>390) {
			skip_temp_record = true;
			target_cur = 800000;
		} else if (board_temp<380) {
			target_cur = 2167000;
		} else if (board_temp>=380 && board_temp<470) {
	...
	} else if (bat_temp>=420 && bat_temp<450) {
		if (board_temp<380) {
			target_cur = 1500000;
		} else if (board_temp>=380 && board_temp<470) {
			if (board_temp<390 && last_board_temp<380) {
	...
	pr_err("[%s] term_volt:%d target_cur:%d board_temp:%d bat_temp:%d last_board_temp:%d "
			"last_bat_temp:%d skip_temp_record:%s \n",
			__func__, term_volt, target_cur, board_temp, bat_temp, last_board_temp,
			last_bat_temp, skip_temp_record?"true":"false");
...
#endif
//modify by jack, jeita policy update(20220524) end
...
static bool cm_charge_check_ato(struct charger_manager *cm)
{
	...
	if(fuel_cap >= ATO_HIGH_CAP && !ato_capacity_limited){
		try_charger_enable(cm, false);
		cm_vbus_enable(cm, true);  //enter the hiz mode
		power_supply_changed(cm->charger_psy);
		ato_capacity_limited = true;
		dev_err(cm->dev, "ATO version stop charger cap = %d\n", fuel_cap);
	//add by jack, 20220514, start/stop charge attr
	} else if(fuel_cap <= ATO_LOW_CAP && ato_capacity_limited){
		cm->charger_enabled = false;
		try_charger_enable(cm, true);
		cm_vbus_enable(cm, false);  //exit the hiz mode
		power_supply_changed(cm->charger_psy);
...
#ifdef WT_COMPILE_FACTORY_VERSION
		ato_capacity_limited = false;
		ret = cm_vbus_enable(cm, false);  //exit hiz mode
		dev_warn(cm->dev, " the usb is plug out!! the hiz mode return is %d\n", ret);
#endif
//add by jack,20220628,add ato electric quantity control,end
	...
	if (cm_check_thermal_status(cm)) {
		dev_warn(cm->dev, "the cm_check_thermal_status is wrong!!\n");
	...
	if (cm_charge_check_ato(cm)) {
		dev_warn(cm->dev, "ATO is limit the charging\n");
		return POWER_SUPPLY_STATUS_NOT_CHARGING;
	}
#endif
...
static bool _cm_monitor(struct charger_manager *cm)
{
	...
	if (cm->battery_status == POWER_SUPPLY_STATUS_CHARGING) {
		...
		#endif
		//Add by xu_shengjie@hoperun.com for VSI-676 setting maximum charge percentage func on 2022-05-31 end
		//Modify by xu_shengjie@hoperun.com for VSI-1649 for when the maximum charging percentage is reached, it's still charging, with a charging icon on 2022-07-07 begin
		#ifdef TARGET_PRODUCT_VSI
		if (!cm->desc->cp.cp_running && !cm_check_primary_charger_enabled(cm)
		    && !cm->desc->force_set_full && !cm->hmd_max_chg_perc_status) {
		#else
		if (!cm->desc->cp.cp_running && !cm_check_primary_charger_enabled(cm)
		    && !cm->desc->force_set_full) {
		#endif
		//Modify by xu_shengjie@hoperun.com for VSI-1649 for when the maximum charging percentage is reached, it's still charging, with a charging icon on 2022-07-07 end
       ...
       struct device *dev, struct device_attribute *attr, char *buf)   
{
	struct charger_regulator *charger
		= container_of(attr, struct charger_regulator,
			       attr_hmd_charge_percentage);
	struct charger_manager *cm = charger->cm;

	dev_info(cm->dev, "get hmd_max_chg_perc=%d\n", cm->hmd_max_chg_perc);
	return sprintf(buf, "%d\n", cm->hmd_max_chg_perc);
}
       ...
       struct device *dev, struct device_attribute *attr,const char *buf, size_t size)
{
	...
	if (buf != NULL && size != 0) {
		dev_info(cm->dev, "buf is %s\n", buf);
		ret = kstrtoul(buf, 10, &val);
		if(ret != 0){
			dev_err(cm->dev, "kstrtoul fail, ret=%d.\n", ret);
			return ret;
		}
		
		if (val >= 100)
		{
			cm->hmd_max_chg_perc = 100;
			dev_info(cm->dev, "%s greater or equal to 100.\n", (int)val);
		}else if(val <= 30)
		{
			cm->hmd_max_chg_perc = 30;
			dev_info(cm->dev, "%s less than or equal to 30.\n", (int)val);
		}else{
			cm->hmd_max_chg_perc = val;
			dev_info(cm->dev, "will set hmd_max_chg_perc=%d\n", cm->hmd_max_chg_perc);
		}
	}
...
//Add by xu_shengjie@hoperun.com for VSI-676 setting maximum charge percentage func on 2022-05-31 begin
#ifdef TARGET_PRODUCT_VSI
	if(((cm->desc->cap)/10 >= cm->hmd_max_chg_perc)&& cm->hmd_max_chg_perc < 100) {
		dev_info(cm->dev,"hmd stop charger");
		dev_info(cm->dev, "hmd_max_chg_perc = %d, cm->desc->cap=%d\n", cm->hmd_max_chg_perc, cm->desc->cap);
		try_charger_enable(cm, false);
		cm->hmd_max_chg_perc_status= 1;
		power_supply_changed(cm->charger_psy);
	}else{
		cm->hmd_max_chg_perc_status= 0;
	}
...
#ifdef WT_COMPILE_FACTORY_VERSION
	cm_vbus_enable(cm, false);  //exit the hiz mode
	dev_err(cm->dev, "lyy - charger_manager_probe exit the hiz mode\n");
#endif
	//add by jack,20220628,add ato electric quantity control,end
	cm_monitor();
